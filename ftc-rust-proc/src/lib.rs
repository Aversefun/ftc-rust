...
use syn::{
    Error, FnArg, Ident, ItemFn, LitStr, PatType, Token, parse::{Parse, Parser}, parse_macro_input, spanned::Spanned
};
...
///
///     ctx.telemetry.addData("Status", "Initalized");
...
///
///     motor.set_power(0.5);
///     ctx.wait_for_seconds(2.0);
///     motor.set_power(0.0);
/// }
///
/// #[ftc(name = "Example: My Iterative Op Mode", iterative, teleop, group = "Example", disabled)]
/// fn my_iterative_op_mode(ctx: &mut ftc::IterativeContext) {
///     // ctx.set_auto_telemetry_update(false); // turn off calling telemetry.update after each iterative callback
///     ctx.on_init(|ctx: &mut ftc::FtcContext| {
///         
///     })
/// }
/// ```
...
pub fn ftc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let java = format!(r#"/* AUTOGENERATED FILE, DO NOT EDIT */
...
"#, if teleop {
    ...
    std::fs::write(java_bindings_dir.join(&class_name), java).unwrap();

    if linear {
        let exported_func_name = format_ident!("Java_{class_name}_runOpMode");
        quote! {
            #func
